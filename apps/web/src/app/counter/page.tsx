...
import { RotateCcw, Plus, Minus, ArrowLeft, Loader2, CheckCircle2, AlertCircle, ExternalLink } from 'lucide-react';
...
export default function CounterDemo() {
  const { activeAddress, isConnected, isConnecting, activeProvider, connect, disconnect, signTransaction } = useWallet();
  ...
  const addLog = useCallback((type: LogEntry['type'], status: LogEntry['status'], details: string, hash?: string) => {
  ...
  const handleAction = async (type: 'increment' | 'decrement' | 'reset') => {
    ...
    try {
      ...
      addLog(type, 'success', `Successfully performed ${type}!`, result.sendTransactionResponse?.hash);
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-6 pb-24 pt-12">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            <div className="relative overflow-hidden rounded-2xl border border-slate-900/80 bg-slate-950/40 p-8 backdrop-blur-md shadow-2xl">
              ...
              <div className="mb-8 flex items-center justify-between border-b border-slate-900 pb-6">
                <div>
                  <h2 className="text-sm font-semibold text-slate-350 tracking-wider uppercase">Wallet Status</h2>
                  <div className="mt-1 flex items-center gap-2 text-xs">
                    {isConnected ? (
                      <>
                        <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
                        <span className="text-slate-400 font-mono">
                          {activeAddress?.substring(0, 6)}...{activeAddress?.substring(activeAddress.length - 4)} ({activeProvider})
                        </span>
                      </>
                    ) : (
                      <>
                ...
                <div>
                  {isConnected ? (
                    <button
                      onClick={disconnect}
                      className="rounded-lg border border-slate-800 bg-slate-900/50 px-4 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-900 hover:text-white transition-all"
                    >
                      Disconnect
                    </button>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="rounded-lg bg-gradient-to-r from-purple-600 to-cyan-500 px-4 py-1.5 text-xs font-semibold text-white shadow-md hover:brightness-110 transition-all flex items-center gap-1.5"
                      >
                        {isConnecting ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Connect Freighter'}
                      </button>
                    </div>
                  )}
                </div>
              ...
              <div className="my-12 flex flex-col items-center justify-center">
                <div className="text-slate-350 text-xs font-semibold uppercase tracking-wider mb-2">Current Count</div>
                <div className="relative">
                  {loadingValue ? (
                    ...
                    <div className="text-7xl sm:text-8xl font-black bg-gradient-to-r from-purple-400 via-pink-400 to-cyan-400 bg-clip-text text-transparent px-4 select-none">
                      {count ?? 0}
                    </div>
                  )}
                </div>
                <button
                  ...
                  className="mt-4 inline-flex items-center gap-1 text-xs text-slate-400 hover:text-white font-mono uppercase tracking-wider"
                >
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-slate-900 pt-8">
                <button
                  ...
                  className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-800/80 bg-slate-950/20 py-4 font-semibold text-slate-200 shadow-inner hover:bg-slate-900/50 transition-all disabled:opacity-40 disabled:pointer-events-none"
                >
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center gap-2 rounded-xl bg-gradient-to-tr from-purple-600/80 to-cyan-500/80 py-4 font-semibold text-white shadow-lg hover:brightness-110 transition-all disabled:opacity-40 disabled:pointer-events-none"
                >
                  {actionLoading ? (
          ...
          <div className="lg:col-span-5">
            ...
            <div className="mb-6 rounded-2xl border border-slate-900 bg-slate-950/20 p-6 backdrop-blur-sm">
              ...
              <div className="mt-4 space-y-3 font-mono text-xs">
                <div>
                  <div className="text-slate-500 uppercase text-[10px]">Contract ID</div>
                  <div className="mt-1 text-slate-300 break-all select-all hover:text-cyan-400 transition-colors">
                    {contractId}
                  </div>
                </div>
                <div>
                  <div className="text-slate-500 uppercase text-[10px]">Network</div>
                  <div className="mt-1 text-slate-300">Stellar Testnet</div>
                </div>
                <div>
                  <div className="text-slate-500 uppercase text-[10px]">Explorer</div>
                  <div className="mt-1">
                    <a
            ...
            <div className="rounded-2xl border border-slate-900 bg-slate-950/40 p-6 backdrop-blur-md">
              ...
              <div className="max-h-[350px] overflow-y-auto space-y-3 pr-2 scrollbar-thin">
                {logs.length === 0 ? (
                  <div className="py-8 text-center text-xs text-slate-600">No transactions recorded yet.</div>
                ) : (
                  logs.map((log) => (
                    <div
                      key={log.id}
                      className="rounded-lg border border-slate-900 bg-slate-950/60 p-3.5 text-xs font-mono"
                    >
                      ...
                      <div className="text-slate-300 text-xs leading-relaxed">{log.details}</div>
                      {log.status === 'pending' && (
